    ...
    class LoginServer : ServerApp
    {
        ...
        private void LoginServer_OnPacket(Client client, Span<byte> packetData)
        {
            ...
            if (opcode == InOpcode.LoginRequest)
            {
                ...
                // 1) Enabled (a flag)
                // 2) A new version ID (?)
                var notice = new OutPacket(OutOpcode.NoticeInfo, client);
                notice.WriteBytePadding(0x415);
                notice.Send();
                ...
                servers.WriteUInt32(100038499); // User ID
                servers.WriteUInt64(17069199591863541); // User authentication token for LoginServer
                servers.WriteHexString("01 01 01 00 01 00 44 6F 6E 20 43 61 76 61 6C 69 65 72 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 53 65 72 76 65 72 20 31 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 B8 0B 03 00 B7 0D");
                servers.Send();
